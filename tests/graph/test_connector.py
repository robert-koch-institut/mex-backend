...
def test_connector_ingest_bench() -> None:
    import json
    import os.path
    import time
    ...
    def get_items() -> list[AnyExtractedModel]:
        models = []
        for file in [
            ...
            if not os.path.isfile(file):  # noqa: PTH113
                continue
    ...
    def benchmark(
        ...
        for _ in range(1):
            ...
            logger.info(f"{id_} posted {nodes} nodes, {edges} edges in {t1}")
    ...
    v2_times, v2_nodes, v2_edges = benchmark("v2", GraphConnector.ingest_v2)
    v1_times, v1_nodes, v1_edges = benchmark("v1", GraphConnector.ingest)
